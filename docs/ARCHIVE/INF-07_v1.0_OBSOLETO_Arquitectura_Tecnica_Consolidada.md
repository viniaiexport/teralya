...
Consolida INF-01 (stack tecnológico), INF-02 (repositorios), INF-03 (entorno cloud), INF-04 (CI/CD), INF-05 (esquema de base de datos) e INF-06 (diccionario de datos) en una única referencia técnica. No modifica ninguna decisión previa, no añade funcionalidad, no diseña APIs ni servicios nuevos. Este documento reemplaza la necesidad de consultar los documentos origen por separado para tener la visión de conjunto; el detalle exhaustivo de cada uno sigue viviendo en su documento fuente para no duplicar contenido.

## Índice

0. Cómo usar este documento · 1. Principios de arquitectura · 2. Stack tecnológico (INF-01) · 3. Repositorios (INF-02) · 4. Entorno cloud y despliegue (INF-03 / INF-04) · 5. Seguridad · 6. Escalabilidad y monitorización · 7. Modelo de datos — resumen (INF-05 / INF-06) · 8. Decisiones y contradicciones cerradas · 9. Estado de cierre y alcance pendiente
...
- Este documento **no reproduce ese detalle** para evitar que ambos contenidos diverjan con el tiempo. Cualquier cambio al modelo de datos se hace primero en esos dos documentos; este maestro solo se actualiza después, a nivel de resumen.
...
Registro histórico consolidado — nada de esto se reabre en este documento, se deja constancia para quien llegue nuevo al proyecto:
...
**Explícitamente fuera de alcance de INF-07** (por instrucción directa del CTO): diseño de APIs, diseño de servicios, y escritura de código nuevo. Estos se abordarán en tareas futuras cuando el CTO lo indique.

*Documento en espera de nuevas instrucciones del CTO.*

---

**NOTA DE SINCRONIZACIÓN:** la versión vigente en Drive es la v1.1 (incorpora §2.1 Internacionalización y §5.1 Verificación de mayoría de edad, aprobadas por el CTO el 09/07/2026). Este archivo del repositorio refleja la v1.0 y está pendiente de actualizarse a v1.1 en un próximo commit.
