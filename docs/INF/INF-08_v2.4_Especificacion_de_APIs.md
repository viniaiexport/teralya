...
**Teralya · Versión 2.4 · 13/07/2026 · APROBADO POR EL CTO**

*INF-08 v2.4: revisión funcional autorizada por DLOG 0019 para formalizar la gestión de Direcciones e Imágenes ya obligatorias en el modelo y en checkout/publicación. Conserva intactos los códigos y rutas de API-001 a API-042 y añade API-043 a API-050. Total: 50 endpoints en 11 módulos. No cambia la topología ni amplía el alcance del MVP.*
Especificación funcional de las APIs del MVP, derivada directamente de los Casos de Uso de CAP-06 y las Pantallas de CAP-05. Documento puramente funcional: no incluye código, no diseña la implementación, no especifica TypeScript ni SQL, no diseña autenticación JWT ni Supabase. 50 endpoints en 11 módulos (API-001 a API-050); API-043 a API-050 completan la implementabilidad end-to-end de requisitos ya aprobados.

## Índice de módulos

1. Autenticación (4) · 2. Bodegas (4) · 3. Vinos (7) · 4. Carrito (5) · 5. Checkout (3) · 6. Pedidos (2) · 7. SubPedidos (3) · 8. Administración (13) · 9. Sistema (1) · 10. Direcciones (4) · 11. Imágenes (4)
...
**Respuesta correcta**
200 OK. En una única transacción: registra el resultado del evento; actualiza `pago.estado` como única fuente económica persistente; conserva los snapshots de envío y facturación congelados al crear el Pedido; congela líneas y precios; aplica el efecto de stock; y crea exactamente un SubPedido por combinación Pedido–Bodega, con solo las líneas de esa bodega y el mismo Pedido/Pago. La confirmación queda disponible para API-018.
...
INF-08 v2.4 queda **APROBADO POR EL CTO** con 50 operaciones en 11 módulos. API-001 a API-042 conservan sus códigos, métodos y rutas. API-043 a API-050 formalizan Direcciones e Imágenes ya exigidas por el MVP. Los dos revisores técnicos emitieron dictamen binario APROBABLE, sin bloqueos.
