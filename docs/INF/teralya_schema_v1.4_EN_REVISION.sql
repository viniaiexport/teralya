...
-- PostgreSQL · Versión 1.4 · Julio 2026 · Estado: EN REVISIÓN
-- Autor: Claude · Basado en: Capítulo 1 - Entidades del MVP
-- Evolución v1.4: conserva las garantías de v1.3 y añade el ledger
--   inmutable carrito_fusion para aplicar DLOG 0017: suma limitada por
--   stock e idempotencia por (comprador_id, fusion_id), sin ampliar MVP.
...
--   resultado guardado y uno distinto con igual clave se rechaza.
-- ---------------------------------------------------------------------
CREATE TABLE carrito_fusion (
    ...
    CONSTRAINT chk_carrito_fusion_payload_hash CHECK (btrim(payload_hash) <> ''),
    CONSTRAINT chk_carrito_fusion_resultado_objeto CHECK (jsonb_typeof(resultado) = 'object')
);
...
-- ---------------------------------------------------------------------
-- 13. SUBPEDIDO  (una fila por bodega dentro de un pedido)
...
-- ---------------------------------------------------------------------
-- 17. AUDITORIA  (polimórfica; nunca se modifica ni se elimina)
