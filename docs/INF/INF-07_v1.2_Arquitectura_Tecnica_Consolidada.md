...
- El **DDL ejecutable completo** (17 tablas, constraints, índices) vive en `teralya_schema_v1.2_EN_REVISION.sql`.
...
- La recuperación de contraseña utiliza `solicitud_recuperacion_password`: el token se persiste exclusivamente como `token_hash`, con estado y fechas de creación, expiración y uso. Los campos de token se eliminan de `usuario`.
...
- Registrar de forma persistente la declaración y la aceptación, junto con sus fechas y la versión de las condiciones aceptadas.
- Mantener `comprador.fecha_nacimiento` como dato obligatorio.

INF-05 v1.2 refleja estas decisiones mediante `declaracion_mayoria_edad`, `declaracion_mayoria_edad_at`, `aceptacion_condiciones_alcohol`, `aceptacion_condiciones_alcohol_at` y `version_condiciones_alcohol`.
...
17 tablas en PostgreSQL 16, en revisión (ver `teralya_schema_v1.2_EN_REVISION.sql` para el DDL e INF-06 v1.1 para el detalle campo a campo):
...
| 16 | `incidencia` | Problema operativo mínimo relacionado con pedido, subpedido, bodega o vino. |
| 17 | `solicitud_recuperacion_password` | Solicitud segura y trazable de recuperación de acceso. |
...
7. **Incidencia operativa (Decisión CEO, 11/07/2026).** Se mantiene como entidad del MVP y se persiste en `incidencia`; reutiliza `auditoria` para el historial mínimo. No incorpora CRM, chat ni tickets avanzados.
8. **Recuperación de contraseña (Decisión CEO, 11/07/2026).** Se separa de `usuario` en `solicitud_recuperacion_password`, con token hasheado, estado y fechas de ciclo de vida.

---

## 9. Estado de cierre y alcance pendiente

**INF-01 a INF-04 permanecen consolidados. INF-05 v1.2 e INF-06 v1.1 están EN REVISIÓN.** INF-07 v1.2 permanece igualmente EN REVISIÓN hasta completar la validación técnica y la aprobación correspondiente.
