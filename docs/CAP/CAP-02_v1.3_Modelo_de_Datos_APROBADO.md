...
| 1.3 | 13/07/2026 | CTO + Arquitecto de Datos | APROBADO | Soporte persistente mínimo de DLOG 0017 mediante CarritoFusion; alineación con INF-05 v1.4 e INF-06 v1.3. |
...
- El email es único sin distinguir mayúsculas y minúsculas.
- Toda cuenta tiene un rol.
- Un usuario con rol bodega pertenece exactamente a una Bodega; comprador y administrador no pueden tener Bodega asociada.
...
- El visitante puede mantener un carrito local no persistente. Tras autenticarse, el carrito se fusiona y persiste para el Comprador. Un Comprador puede tener muchos Carritos a lo largo del tiempo, pero solo uno activo.
...
- La comisión es obligatoria cuando la Bodega está aprobada o activa; la aprobación solo puede atribuirse a un Administrador activo.
...
- SKU y estado de publicación, incluido `pendiente_revision`.
...
- La Bodega crea y edita sus vinos y solicita publicación; la solicitud válida cambia a `pendiente_revision`.
- Solo Administración publica o despublica.
...
- El carrito de visitante permanece local y no tiene fila persistente.
- Cada Carrito persistente pertenece a un Comprador.
...
- Un Carrito válido inicia un Checkout y puede convertirse en un único Pedido.

### Reglas de integridad

- Puede contener vinos de varias bodegas.
- El carrito local del visitante se fusiona al autenticarse; Checkout siempre requiere Comprador autenticado.
...
- La repetición del Checkout sobre el mismo Carrito devuelve el Pedido ya creado y no genera otro.
- El total guardado en carrito no sustituye al cálculo definitivo del Checkout.

### Registro técnico de fusión
...
- No confirma el Pedido antes de la confirmación fiable y firmada del webhook de Stripe.
- La sesión de Stripe Checkout se reutiliza mientras sea válida.
...
- Cada Pedido referencia obligatoriamente una Dirección activa de envío y otra de facturación pertenecientes al Comprador y habilitadas para el uso correspondiente; pueden ser la misma si cumple ambos usos.
...
- Referencias interna y de Stripe, incluida la sesión de Stripe Checkout y su expiración.
...
- Solo el webhook firmado e idempotente de Stripe puede confirmar el Pago y la compra.
- El total cobrado debe corresponder al total definitivo del Pedido.
- El total repartido y el total reembolsado no pueden superar el total cobrado.
- El pago no referencia una única Cuenta Stripe Connect porque un Pedido puede contener varias bodegas.
- `pago.estado` es la única fuente persistente del estado económico; Pedido no duplica esos datos.
- El total cobrado y el total del Pedido deben coincidir en ambos sentidos.
- Una misma sesión de Stripe Checkout no puede vincularse a más de un Pago.
- La repetición de un evento Stripe ya registrado no produce un segundo efecto comercial.
...
- Comprador y Carrito de origen único.
- Líneas de producto congeladas.
- Subtotal, envío, impuestos, descuentos reservados y total.
- Relación con Pago, sin duplicar estado, método ni fecha del pago.
...
- Cada Pedido pertenece a un Comprador y procede de un único Carrito.
...
- Un Carrito solo puede originar un Pedido; los reintentos devuelven el existente.
...
- Cada SubPedido se relaciona con el Pago del mismo Pedido.
- Cada SubPedido contiene una o muchas líneas del mismo Pedido y de esa Bodega; cada Vino de línea pertenece a dicha Bodega.
- Un Pedido genera exactamente un SubPedido por cada Bodega implicada.
- Una Bodega puede recibir muchos SubPedidos.
...
- No puede existir más de un SubPedido para la misma combinación Pedido–Bodega.
...
- El ciclo permitido es `abierta → en_revision → resuelta → cerrada`, sin saltos ni reaperturas.
- Los cambios de estado de Incidencia quedan auditados automáticamente. Las demás actuaciones relevantes deben generar su registro de Auditoría.
...
## Evento Webhook Stripe

Registra cada evento recibido desde Stripe mediante un identificador único. Su finalidad es impedir procesamiento duplicado, conservar el resultado técnico y vincular el evento con el Pago cuando corresponda. Es una entidad persistente de soporte de API-029 y no una funcionalidad comercial independiente.
...
- Cada línea de Pedido coincide simultáneamente con el Pedido y la Bodega de su SubPedido, y su Vino pertenece a esa misma Bodega.
...
- Pago es la única fuente persistente del estado económico.
...
- La Incidencia sigue exclusivamente `abierta → en_revision → resuelta → cerrada`.
...
- Las asociaciones polimórficas de Direcciones e Imágenes se validan mediante los mecanismos técnicos aprobados y se protegen frente a borrados que generen referencias huérfanas.
- Auditoría conserva la relación funcional por tipo e identificador y sus registros son inmutables.
...
Stripe Checkout procesa el cobro único. La sesión se reutiliza mientras sea válida. Cada evento de webhook se registra una sola vez y el webhook firmado constituye la autoridad de confirmación que actualiza Pago. La Cuenta Stripe Connect de cada Bodega permite el reparto correspondiente y la retención de la comisión de Teralya.
...
| Representación persistente y relaciones | INF-05 v1.4, APROBADO POR CTO |
| Significado de campos y reglas técnicas | INF-06 v1.3, APROBADO POR CTO |
| Pago como única fuente económica | Decisión 0014 |
...
| Contratos operativos del MVP | INF-08 v2.3, APROBADO POR CTO |
| Administrador como rol sin tabla propia | Decision Log 0003 e INF-05 v1.4, APROBADO POR CTO coordinada |
| Checkout e Idioma sin tabla independiente | CAP-01 e INF-05 v1.4, APROBADO POR CTO coordinada |
| Pedido completo y SubPedidos por bodega | CAP-01, INF-05 v1.4 e INF-06 v1.3, APROBADO POR CTO coordinada |
| Evento Webhook Stripe como soporte técnico | INF-05 v1.4, INF-06 v1.3 e INF-08 v2.3 API-029, APROBADO POR CTO |
...
CAP-02 v1.3 consolida el modelo de datos funcional del MVP y mantiene la separación entre la experiencia comercial del comprador y la operación multi-bodega interna.
...
El documento queda **APROBADO POR EL CTO**. Las reglas persistentes se apoyan en INF-05 v1.4 e INF-06 v1.3 aprobados.
