...
import type {
  BodegaPerfil,
  BodegasRepository,
} from "../src/modules/bodegas/bodegas.repository.js";
...
function setup() {
  ...
  const service = new BodegasService(
    repository as unknown as BodegasRepository,
  );
...
describe("API-006/API-031 — perfil propio de bodega", () => {
  ...
  it("distingue bodega inexistente de bodega no operativa al consultar", async () => {
    ...
    repository.obtenerPerfil
      ...
      .mockResolvedValueOnce({ ...PERFIL, estado: "suspendida" });
    await expect(service.obtenerPerfilPropio(PERFIL.id)).rejects.toBeInstanceOf(
      NotFoundException,
    );
    await expect(service.obtenerPerfilPropio(PERFIL.id)).rejects.toBeInstanceOf(
      ForbiddenException,
  ...
  it("rechaza PATCH vacío y no llama al repositorio", async () => {
    const { repository, service } = setup();
    await expect(
      service.actualizarPerfilPropio(PERFIL.id, {}),
  ...
  it("actualiza exclusivamente la bodega derivada del actor", async () => {
    const { repository, service } = setup();
    repository.obtenerPerfil.mockResolvedValue(PERFIL);
    repository.actualizarPerfil.mockResolvedValue({
      ...PERFIL,
      nombre_comercial: "Nombre nuevo",
    });

    const result = await service.actualizarPerfilPropio(PERFIL.id, {
      nombre_comercial: "Nombre nuevo",
    });

    expect(repository.actualizarPerfil).toHaveBeenCalledWith(PERFIL.id, {
      nombre_comercial: "Nombre nuevo",
    });
  ...
  it("traduce colisiones únicas a 409", async () => {
    const { repository, service } = setup();
    repository.obtenerPerfil.mockResolvedValue(PERFIL);
    repository.actualizarPerfil.mockRejectedValue(
      new EmailBodegaYaRegistradoError(),
    );
    await expect(
      service.actualizarPerfilPropio(PERFIL.id, {
        email_principal: "ocupado@example.com",
      }),
