...
describe('API-028 dashboard administrativo E2E',()=>{
  ...
  beforeAll(async()=>{
    ...
    const order=async(state:string,total:number)=>required((await pool.query<{id:string}>(`INSERT INTO pedido(numero_pedido,comprador_id,subtotal,gastos_envio,impuestos,descuentos,total,direccion_envio_snapshot,direccion_facturacion_snapshot,estado) VALUES($1,$2,$3,0,0,0,$3,'{}','{}',$4) RETURNING id`,[`TER-${randomUUID()}`,buyer,total,state])).rows[0]).id;
