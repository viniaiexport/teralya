...
describe('API-028 dashboard administrativo E2E',()=>{
  ...
  it('agrega pagos capturados hoy y cuenta únicamente pendiente_pago',async()=>{const response=await request(app.getHttpServer() as Server).get('/admin/dashboard').set({Authorization:`Bearer ${adminToken}`}).expect(200);const body=response.body as {ventas_dia:{importe:string;moneda:string;num_pedidos:number};pedidos_pendientes:number};expect(body.ventas_dia.moneda).toBe('EUR');expect(Number(body.ventas_dia.importe)).toBeGreaterThanOrEqual(37.25);expect(body.ventas_dia.num_pedidos).toBeGreaterThanOrEqual(1);expect(body.pedidos_pendientes).toBeGreaterThanOrEqual(1);expect(Object.keys(response.body as object).sort()).toEqual(['pedidos_pendientes','ventas_dia']);});
