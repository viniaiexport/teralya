...
export class AwsObjectStorageService implements ObjectStorageService{
 ...
 constructor(config:ConfigService){this.bucket=config.getOrThrow<string>('OBJECT_STORAGE_BUCKET');const endpoint=config.get<string>('OBJECT_STORAGE_ENDPOINT');this.client=new S3Client({region:config.getOrThrow<string>('OBJECT_STORAGE_REGION'),...(endpoint===undefined?{}:{endpoint}),forcePathStyle:config.get<boolean>('OBJECT_STORAGE_FORCE_PATH_STYLE')??false,credentials:{accessKeyId:config.getOrThrow<string>('OBJECT_STORAGE_ACCESS_KEY_ID'),secretAccessKey:config.getOrThrow<string>('OBJECT_STORAGE_SECRET_ACCESS_KEY')}});}
