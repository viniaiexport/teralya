...
export class SubpedidosRepository {
  ...
  cambiarEstado(id:string,bodegaId:string,userId:string,destination:SubOrderState):Promise<TransitionResult>{return this.database.withTransaction(async c=>{
    ...
    await c.query(`UPDATE subpedido SET estado=$2,fecha_ultimo_cambio_estado=now(),updated_at=now(),fecha_preparacion=CASE WHEN $2='en_preparacion' AND fecha_preparacion IS NULL THEN now() ELSE fecha_preparacion END,fecha_envio=CASE WHEN $2='enviado' AND fecha_envio IS NULL THEN now() ELSE fecha_envio END,fecha_entrega_real=CASE WHEN $2='entregado' AND fecha_entrega_real IS NULL THEN now() ELSE fecha_entrega_real END WHERE id=$1`,[id,destination]);
    const parent=deriveOrderState(siblings.rows.map(row=>row.id===id?destination:row.estado));await c.query('UPDATE pedido SET estado=$2,updated_at=now(),fecha_cierre=CASE WHEN $2 IN (\'entregado\',\'cancelado\') THEN coalesce(fecha_cierre,now()) ELSE fecha_cierre END WHERE id=$1',[current.pedido_id,parent]);
