...
export default async function CheckoutPage({ searchParams }: Props) {
  ...
  return <main className="checkout-page"><nav aria-label="Migas de pan" className="breadcrumbs"><Link href="/carrito">Carrito</Link><span aria-hidden="true">/</span><span aria-current="page">Checkout</span></nav><header className="checkout-heading"><p className="eyebrow">PT-COM-003</p><h1>Direcciones y resumen</h1><p>Revisa dónde debe enviarse y facturarse tu pedido antes de pasar a Stripe.</p></header>
    ...
    <div className="checkout-layout"><section className="checkout-addresses">
      {addresses.length === 0 ? <section className="catalog-message"><h2>Añade tu primera dirección</h2><p>Necesitamos una dirección válida para preparar el pedido.</p></section> : <form action={prepareCheckoutAction} id="checkout-form">
        <fieldset><legend>Dirección de envío</legend>{shipping.length === 0 ? <p className="cart-warning">No tienes una dirección válida para envío.</p> : shipping.map((address, index) => <AddressChoice address={address} defaultChecked={address.es_principal || index === 0} key={address.id} name="direccion_envio_id"/>)}</fieldset>
        <fieldset><legend>Dirección de facturación</legend>{billing.length === 0 ? <p className="cart-warning">No tienes una dirección válida para facturación.</p> : billing.map((address, index) => <AddressChoice address={address} defaultChecked={address.es_principal || index === 0} key={address.id} name="direccion_facturacion_id"/>)}</fieldset>
      </form>}
      ...
      <AddressForm idempotencyKey={randomUUID()}/>
    </section>
    <aside className="checkout-summary"><h2>Resumen del pedido</h2>{cart.items.map((item) => <div className="checkout-line" key={item.id}><span>{item.cantidad} × {item.vino.nombre_comercial}</span><strong>{formatEuro(item.importe_total)}</strong></div>)}<dl><div><dt>Subtotal</dt><dd>{formatEuro(cart.subtotal)}</dd></div><div><dt>Envío</dt><dd>{formatEuro(cart.gastos_envio)}</dd></div><div><dt>Descuentos</dt><dd>−{formatEuro(cart.descuentos)}</dd></div><div className="summary-total"><dt>Total</dt><dd>{formatEuro(cart.total)}</dd></div></dl><p>El backend volverá a validar precios, publicación y stock antes de crear el Pedido.</p><button className="button primary cart-checkout" disabled={shipping.length === 0 || billing.length === 0} form="checkout-form" type="submit">Preparar pedido</button></aside></div>
  </main>;
}
