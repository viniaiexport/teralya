...
import { getBuyerOrder, listBuyerOrders } from '../src/lib/orders/server';

const orderId = '22222222-2222-4222-8222-222222222222';

beforeEach(() => {
  vi.clearAllMocks();
  readSessionIdentity.mockResolvedValue({ usuario_id: 'buyer', rol: 'comprador' });
...
describe('contrato de pedidos del comprador FE-007', () => {
  it('consulta únicamente pedidos propios mediante API-019', async () => {
    apiRequest.mockResolvedValue({ items: [], page: 2, page_size: 20, total_items: 0, total_pages: 0 });
    await listBuyerOrders(2);
    expect(apiRequest).toHaveBeenCalledWith('/pedidos?page=2&page_size=20', { method: 'GET', token: 'opaque-token' });
  });
  ...
  it('normaliza paginación no válida antes de llamar al backend', async () => {
    ...
    expect(apiRequest).toHaveBeenCalledWith('/pedidos?page=1&page_size=20', { method: 'GET', token: 'opaque-token' });
  });
  ...
  it('consulta el detalle propio mediante API-020', async () => {
    ...
    expect(apiRequest).toHaveBeenCalledWith(`/pedidos/${orderId}`, { method: 'GET', token: 'opaque-token' });
  });

  it('rechaza identificadores manipulados sin consultar la API', async () => {
    await expect(getBuyerOrder('../admin/pedidos')).rejects.toThrow('Pedido inválido');
    expect(apiRequest).not.toHaveBeenCalled();
  });

  it('bloquea roles distintos del comprador', async () => {
    readSessionIdentity.mockResolvedValue({ usuario_id: 'winery', rol: 'bodega', bodega_id: 'winery-id' });
